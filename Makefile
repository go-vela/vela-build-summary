...
LD_FLAGS = -X github.com/go-vela/vela-build-summary/version.Commit=${GITHUB_SHA} -X github.com/go-vela/vela-build-summary/version.Date=${BUILD_DATE} -X github.com/go-vela/vela-build-summary/version.Go=${GOLANG_VERSION} -X github.com/go-vela/vela-build-summary/version.Tag=${GITHUB_TAG}
...
build:
	@echo
	@echo "### Building release/vela-build-summary binary"
	GOOS=linux CGO_ENABLED=0 \
		go build -a \
		-ldflags '${LD_FLAGS}' \
		-o release/vela-build-summary \
		github.com/go-vela/vela-build-summary/cmd/vela-build-summary
...
build-static:
	@echo
	@echo "### Building static release/vela-build-summary binary"
	GOOS=linux CGO_ENABLED=0 \
		go build -a \
...
build-static-ci:
	@echo
	@echo "### Building CI static release/vela-build-summary binary"
	@go build -a \
		-ldflags '-s -w -extldflags "-static" ${LD_FLAGS}' \
		-o release/vela-build-summary \
		github.com/go-vela/vela-build-summary/cmd/vela-build-summary
...
docker-build:
	@echo
	@echo "### Building vela-build-summary:local image"
...
docker-test:
	@echo
	@echo "### Testing vela-build-summary:local image"
	@docker run --rm \
		vela-build-summary:local
...
docker-run:
	@echo
	@echo "### Executing vela-build-summary:local image"
	@docker run --rm \
